...
import type { ScoredCard, QuizAnswers } from "./types";
import {
  Q1_LABELS,
  Q2_LABELS,
  Q3_LABELS,
...
function buildReasoningPrompt(card: ScoredCard, answers: QuizAnswers): string {
  ...
  return `You are a friendly credit card advisor. The user has these priorities:

- Credit situation: ${Q1_LABELS[answers.q1]}
- Balance behavior: ${Q2_LABELS[answers.q2]}
- Primary goal: ${Q3_LABELS[answers.q3]}
...
Write 2-3 sentences explaining why this card fits the user. Be specific — cite actual features that match their stated priorities. Do NOT list features; explain why those features matter given what they said. If there's a tradeoff (e.g., fee in exchange for rewards), acknowledge it honestly. Do not exaggerate or make up features.

Write in second person ("you'll"), conversational but professional tone. No emojis, no bullet points, no headers.`;
}
...
function buildFallbackReasoning(card: ScoredCard, answers: QuizAnswers): string {
  ...
  } with ${fee}. Given your goal of ${Q3_LABELS[answers.q3].toLowerCase()}, this card's features align well with your stated priorities. Verify current terms directly with ${c.issuer} before applying.`;
}
